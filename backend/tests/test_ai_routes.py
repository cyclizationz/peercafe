...
def mock_recommender():
    ...
    with patch('routes.ai_routes.get_recommender') as mock_get:
        instance = Mock()
        instance.get_recommendations.return_value = "Here are some great Italian restaurants downtown with vegetarian options!"
...
def test_get_recommendations_success(mock_recommender, clear_sessions):
    ...
    response = client.post(
        ...
        json={"query": "I want Italian food downtown"}
    )
    ...
    mock_recommender.get_recommendations.assert_called_once_with("I want Italian food downtown")
...
def test_get_recommendations_recommender_not_configured(mock_recommender_none, clear_sessions):
    """Test when recommender initialization fails"""
    response = client.post(
        "/api/ai/recommendations",
        json={"query": "I want Italian food"}
    )
...
def test_chat_new_session(mock_chatbot, clear_sessions):
    ...
    response = client.post(
        "/api/ai/chat",
        json={"message": "I'm looking for dinner", "session_id": None}
    )
...
def test_chat_existing_session(mock_chatbot, clear_sessions):
    ...
    response1 = client.post(
        "/api/ai/chat",
        json={"message": "I want Italian food", "session_id": None}
    )
...
def test_chat_with_custom_session_id(mock_chatbot, clear_sessions):
    ...
    response = client.post(
        "/api/ai/chat",
        json={"message": "Hello", "session_id": custom_session_id}
    )
...
def test_list_sessions_with_active_sessions(mock_chatbot, clear_sessions):
    ...
    for i in range(3):
        response = client.post(
            "/api/ai/chat",
            json={"message": f"Hello {i}", "session_id": None}
        )
...
def test_full_conversation_flow(mock_chatbot, clear_sessions):
    ...
    response1 = client.post(
        "/api/ai/chat",
        json={"message": "I'm hungry", "session_id": None}
    )
    session_id = response1.json()["session_id"]
    ...
    response2 = client.post(
        "/api/ai/chat",
        json={"message": "Italian food", "session_id": session_id}
    )
...
def test_session_isolation(mock_chatbot, clear_sessions):
    ...
    response1 = client.post(
        "/api/ai/chat",
        json={"message": "Session 1", "session_id": None}
    )
    session1_id = response1.json()["session_id"]
    
    response2 = client.post(
        "/api/ai/chat",
        json={"message": "Session 2", "session_id": None}
    )
