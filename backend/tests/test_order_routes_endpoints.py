...
def test_verify_delivery_code_awards_loyalty_points_to_delivery_person(monkeypatch):
    ...
    mock_update_response.data = [{**order_data, "status": orr.OrderStatus.DELIVERED.value, "delivery_code_used": True}]
    
    # Mock refetch response
    mock_refetch_response = MagicMock()
    mock_refetch_response.data = [{**order_data, "status": orr.OrderStatus.DELIVERED.value, "delivery_code_used": True}]
    ...
    monkeypatch.setattr(orr, "_normalize_single_order", AsyncMock(return_value=order_data))
...
def test_verify_delivery_code_loyalty_points_calculation_various_amounts(monkeypatch):
    ...
    for total_amount, expected_points in test_cases:
        ...
        call_args = mock_loyalty_update.call_args[0]
        assert call_args[2] == expected_points, f"Failed for amount {total_amount}: expected {expected_points}, got {call_args[2]}"
